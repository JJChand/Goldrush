...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=MDMzZTBhYTQ2MmU5MzVlOTkxMTNlZGE4ZTllMGQ2MjRfYmE2NjQ0YzllMWE2ZjY1MzgxM2ExOWQ2NzNmYzU1ODNfSUQ6NzY2MDA5MzQxNzA2MTUwMjE0Ml8xNzg2NTQyNzg1OjE3ODY2MjkxODVfVjM)
...
🔗https://l4x826wg3c\.feishu\.cn/share/base/form/shrcnGy5v0lASkGUXPvIR1Vo1Db
...
- **更多赛事介绍，请点击：**https://mp\.weixin\.qq\.com/s/cP3MhQRIQNYhQag2bn4Enw
...
- 报名链接：https://app\.mokahr\.com/campus\_apply/ubiquantrecruit/41168?hash=%23%2Fjob%2Fffa5ccb4\-8907\-4f5a\-9965\-dd06a5a77d87%3Ffrom%3Dqrcode%26isRecommendation%3Dfalse
...
区域2：左上13*4（[0,0]-[3,12]）
区域3：左下13*4（[4,0]-[16,3]）
区域4：右下13*4（[13，4]-[16,16]）
区域5：右上13*4（[0,13]-[12,16]）
...
conda路径：/share/opt/anaconda3，conda环境名称为game。如果你需要的一些库没有提供，可以在这里提交申请：https://l4x826wg3c\.feishu\.cn/share/base/form/shrcnGy5v0lASkGUXPvIR1Vo1Db
...
A：同学你访问的是开发机，这台机器应该通过ssh来访问而不是用浏览器访问。通过浏览器访问的比赛网站是http://47\.103\.127\.219。然后也可以排查一下是不是自己开了梯子之类的。
...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZjRkYzNiZGUxMmRiZGRhYWVhMWQ4YTU2M2NkZjdmMGFfNDBjZGQ4YjVjNWYwOTI4ZGI1MzA2ODhkNGRlYTg3ZDNfSUQ6NzY2NTE5NjYwNTE1MDMwMTEzMl8xNzg2NTQyNzg1OjE3ODY2MjkxODVfVjM)
...
# 附加说明
三、比赛规则
模拟赛：选手提前熟悉初赛流程。选手自愿参加，不会影响初赛成绩。
初赛：会使用2-5张新的地图。
初赛对局规则：选手提交截止后，启动对局。每张地图每两个队伍之间会交换出生点各对局1次。
初赛排名规则：优先按照每个队伍在初赛期间的胜率排名，如果两个队伍胜率相等，则根据每一局的P90延迟的中位数进行排序。如果仍然相等，则依次按照每次对局的P95，P99延迟中位数排序。
四、地图和游戏机制
游戏的数值和金币生成逻辑和公测基本一致。
地图保证的性质: 障碍物的位置至少满足上下对称或者左右对称中的一条。
五、 Q&A（问卷星调研问题回复）
Q：L1 cache是否会均分？
A：初赛和公测阶段保持一致。

Q：组织方主观上是否希望我们对不同地图的疏密程度编写不同策略，还是希望尽可能用一种策略适应不同的地图类型?
A：只要能战胜对手，并且没有违反比赛规则，什么样的策略都可以。

Q：决赛将在两个队伍的策略之间比赛几场？使用几张地图？
A：我们会根据大家的策略实际情况进行调整，降低偶然性。

Q：初赛提交的代码是否与公测完全独立？
A: 模拟赛，初赛的策略都是和公测独立的，公测无法选择别人在模拟赛和初赛提交的策略对战。
...
Q：比赛中两支队伍的策略会分配独立的计算资源吗？会不会出现一个策略占满所有计算资源，导致另一策略出现问题？多线程支持？
A：初赛不会分配独立的计算通道；我们会尽量减少类似现象的发生。不禁止多线程，但是禁止选手策略设置CPU亲和性，同一时刻最多允许占满4个线程。

Q:  不卷成绩，但降低P90专门设计出来的策略，去搏极速奖，这种情况怎么判定?
A：极速奖的前提是能进决赛，策略太弱的话大概率进不了决赛。

Q：决赛相关的问题
A:   初赛选出进入决赛的16个队伍后，我们会在决赛群里发布。

六、线上答疑会小结（附视频）
暂时无法在飞书文档外展示此内容
1. 初赛竞技规则与环境设定的问题
1.1 排名机制与地图特性的问题
- 排名规则：根据胜率进行排名，胜率相同则比较 P90 延迟，若仍相同则依次比较 P95、P99 延迟。
- 地图对称性：初赛将使用全新的地图，障碍物设计至少满足上下对称或左右对称，金币生成规律保持对称性。
- 参数一致性：游戏的大部分核心参数保持和公测期间一致。
1.2 网络和资源限制
- 多线程限制：不严格禁止多线程，但禁止设置 CPU 亲和性，且同一时刻最多占用 4 个 CPU 核心，禁止常驻线程。若策略运行时间过长且占用资源过多，可能被视为违规。
- 网络限制：禁止策略联网，任何尝试联网传输数据的行为均被视为作弊。
2. 策略更新与对局流程
2.1 策略更新窗口的问题
- 更新机制：每张地图对局结束后，官方会预留至少6 小时的策略修改时间。选手可在此期间更新代码，新策略将应用于下一张地图的对局。
- 时间安排：6 小时的更新窗口会尽量安排在北京时间的“阳间”时段（如白天）。具体截止时间将在每张地图跑完后公布。
2.2 对局执行方式的问题
- 离线运行：所有对局均为离线运行。在全部对局跑完之前，系统不会显示任何结果，选手也无法查看回放或日志。
- 出生点交换：每两个队伍之间会对局两次，分别交换主对角线和副对角线的出生点，以确保公平性。
3. 关于编译环境与依赖的专项答疑
Q：如果担心本地编译环境与官方不一致导致哈希值不同，能否将整个编译器环境打包上传？
A：不建议打包整个编译器（体积过大），官方会提供基础编译环境。若有特殊依赖库，可打包库文件，但需提前整理需求与官方沟通。
4. 关于策略更新窗口的专项答疑
Q：6 小时的策略更新窗口具体如何运作？更新后的策略是否会对已跑完的地图重新评分？
A：该窗口是在一张地图跑完后开启。更新后的策略仅用于后续的新地图，已跑完的地图成绩不会重新计算。
5. 关于地图与参数的专项答疑
Q：模拟赛是否会使用新地图？
A：模拟赛会复用现有的公测地图（如地图三），目的是熟悉流程；新地图将用于初赛。
Q：金币刷新规律是否会改变？
A：金币刷新规律不会改变，但具体细节（如与地图关联的位置）会随地图变化而调整。
6. 其它
Q：如何计算so的哈希值？
A：在九坤编译机上运行：
sha256sum player.so （player.so可以替换为你策略的具体so的名称）
输出：
b9a020d3aa79d54f99829a1893229ef8e7e7cea9333a3ed119741d8214bd6010  player.so
红色部分即为so的哈希值。

*组委会对比赛有完全解释权，一切以九坤投资提供的文档为准，欢迎大家随时关注文档
