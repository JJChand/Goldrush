...
| 当前 v2（beam width 4） | ~715 us | ~840 us | **~2.31x faster** |

这里测的是可复现的策略热路径，不是官方服务器成绩，也不是胜率回测；不同
机器的绝对值不可直接横比。README 原先记录 v1 在另一台机器约 0.9 ms P90，
按本地相对加速比估算，v2 在同机约为 **380–450 us**，仍应以上传后的官方
P90 为准。

宽度 4 是按本次“延迟第一”要求选择的默认值。本地宽度 6 的 P90 约为
940 us，多约 100 us；端点多样性剪枝后，其内部规划分数优势随负载约为
0–0.8 个百分点，尚未用真实固定先手自对弈证明这部分分数足以抵消后手风险。
...
热点学习收益，以及 C++ `.so` 移植。若目标是几十到一百微秒，Python 热路径
继续小修的收益有限，下一步应拿官方 `game_api.h` 做 C++ 端口并配对回测。
